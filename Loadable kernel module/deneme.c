...
static int variable = 1000;


int myAtoi(const char* str) //from www.geeksforgeeks.org/
{
...
char* itoa(int num, char* str, int base) //from  geeksforgeeks.org/
{
    int i = 0;
...
// kernel string allocation
	...
	if(copy_from_user(tmp_buf, buffer, msg_len)){
		kfree(tmp_buf);
		return EFAULT;
	}
	...
	intvalue=myAtoi(tmp_buf); //Kullanıcıdan alınan string tmp_buf'ı integer'a çevirmek için
	variable = variable+intvalue;
	itoa(variable,tmp_buf,10);//Integer olarak elde edilen değeri String olarak saklamak için.Variable değerini tmp_buf içerisine atar.
	//pr_info(" %d\n",variable);
	//pr_info(" %s\n",tmp_buf);



	cpy_len = (msg_len < MAX_MODULE_BUFFER) ? msg_len : MAX_MODULE_BUFFER;
...
static int module_file_show(struct seq_file *m,void *v){
...
{
	return single_open(file,module_file_show,NULL);
}
...
{
	printk(KERN_INFO "HW is started!!\n");
